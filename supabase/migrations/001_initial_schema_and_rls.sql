-- RentyVest initial schema with custom RLS for Privy token exchange
-- JWT `sub` carries the Privy DID (e.g. did:privy:...) minted by core-api
...
COMMENT ON FUNCTION public.requesting_user_id() IS
  'Returns the Privy DID from the JWT sub claim in request.jwt.claims.';
...
CREATE INDEX IF NOT EXISTS pledges_user_id_idx
  ON public.pledges (user_id);

CREATE INDEX IF NOT EXISTS pledges_property_id_idx
  ON public.pledges (property_id);
...
CREATE INDEX IF NOT EXISTS nfts_owner_id_idx
  ON public.nfts (owner_id);

CREATE INDEX IF NOT EXISTS nfts_property_id_idx
  ON public.nfts (property_id);

CREATE UNIQUE INDEX IF NOT EXISTS nfts_canton_contract_id_unique_idx
